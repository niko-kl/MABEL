...
int acc_calibration_value = 1024; //Enter the accelerometer calibration value
...
void setup()
{
  ...
  delay(2000);
  Serial.println("Init I2C and TIMER variables");
  delay(500);
  TWBR = 12;          //Set the I2C clock speed to 400kHz
  
  //By default the MPU-6050 sleeps. So we have to wake it up.
  Serial.print("Starting gyro communication...");
  delay(500);
...
if (Serial.available())
  ...
  Serial.println(throttle_left_motor_memory);
  ...
  if(throttle_counter_left_motor > throttle_left_motor_memory){             //If the number of loops is larger then the throttle_left_motor_memory variable
    ...
    if(throttle_left_motor_memory < 0){                                     //If the throttle_left_motor_memory is negative
      digitalWrite(5, LOW);                                                  //Set output 3 low to reverse the direction of the stepper controller (5)
      throttle_left_motor_memory *= -1;                                     //Invert the throttle_left_motor_memory variable
    }
    else digitalWrite(5, HIGH);                                             //Set output 3 high for a forward direction of the stepper motor (5)
  }
  delay(1000);
  Serial.print("going for loops:");
  Serial.println(abs(throttle_left_motor/2));
  for(int i = 0; i < abs(throttle_left_motor/2); i++) {
    digitalWrite(2,HIGH); 
